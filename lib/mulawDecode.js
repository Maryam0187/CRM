...
function decodeMulawBase64ManyToFloat32(payloads) {
  if (!payloads?.length) return new Float32Array(0);
  if (payloads.length === 1) return decodeMulawBase64ToFloat32(payloads[0]);
  let total = 0;
  const parts = payloads.map((payload) => {
    const decoded = decodeMulawBase64ToFloat32(payload);
