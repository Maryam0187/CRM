...
const createDocxBlob = async ({ title = 'Document', generatedAt = new Date(), meta = [], sections = [] }) => {
  ...
  const BODY_TEXT_SIZE = 22; // 11pt in docx half-point units
  const SECTION_HEADING_SIZE = 24; // 12pt
  const TITLE_TEXT_SIZE = 32; // 16pt
  ...
  const paragraphFromMultilineLabel = (label, rawText) => {
    ...
    if (label != null && String(label).trim() !== '') {
      runs.push(new TextRun({ text: `${String(label).trim()}: `, bold: true, size: BODY_TEXT_SIZE }));
    }
    runs.push(new TextRun({ text: lines[0] || '', size: BODY_TEXT_SIZE }));
    for (let i = 1; i < lines.length; i += 1) {
      runs.push(new TextRun({ break: 1 }));
      runs.push(new TextRun({ text: lines[i] || '', size: BODY_TEXT_SIZE }));
    }
  ...
  const children = [
    new Paragraph({
      ...
      children: [new TextRun({ text: title, bold: true, size: TITLE_TEXT_SIZE })]
    }),
    new Paragraph({
      spacing: paragraphSpacing,
      children: [
        new TextRun({ text: 'Generated: ', bold: true, size: BODY_TEXT_SIZE }),
        new TextRun({ text: String(generatedAt.toLocaleString()), size: BODY_TEXT_SIZE })
      ]
    })
  ];
  ...
  meta.forEach((line) => {
    if (line && String(line).trim()) {
      children.push(
        new Paragraph({
          spacing: paragraphSpacing,
          children: [new TextRun({ text: String(line).trim(), size: BODY_TEXT_SIZE })]
        })
      );
    }
  });
    ...
    if (section.heading) {
      children.push(
        new Paragraph({
          ...
          spacing: paragraphSpacing,
          children: [new TextRun({ text: String(section.heading), bold: true, size: SECTION_HEADING_SIZE })]
        })
      );
    }
      ...
      if (multiline) {
        ...
        if (indentMultilineBody) {
          children.push(
            new Paragraph({
              ...
              children: [new TextRun({ text: `${String(label).trim()}:`, bold: true, size: BODY_TEXT_SIZE })]
            })
          );
          const lines = normalized.split('\n');
          const bodyRuns = [];
          lines.forEach((line, i) => {
            ...
            bodyRuns.push(new TextRun({ text: trimmedLeadTabs, size: BODY_TEXT_SIZE }));
          });
          children.push(
            new Paragraph({
              spacing: paragraphSpacing,
        ...
        children.push(
          new Paragraph({
            spacing: paragraphSpacing,
            children: [
              new TextRun({ text: `${label}: `, bold: true, size: BODY_TEXT_SIZE }),
              new TextRun({ text: String(normalized), size: BODY_TEXT_SIZE })
            ]
          })
        );
      }
...
export const downloadSaleDoc = async ({
  ...
  if (sale || customer || card || bank || paymentsInfo) {
    ...
    const titleWithSaleId = `Sale #${saleId} by ${agentName} of ${customerName}`;
