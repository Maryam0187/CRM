...
const normalizePhoneForStorage = (phoneNumber) => {
...
const normalizePhoneForMatching = normalizePhoneForStorage;
 ...
 * @param {string} conferenceName - Conference friendly name (e.g., 'call-1')
 * @param {string} conferenceSid - Optional: Conference SID (if provided, conferenceName is ignored)
 * @returns {Promise<Array>} Array of participant objects with status
 */
const getConferenceParticipants = async (conferenceName, conferenceSid = null) => {
 ...
 * @param {string} callSid - Call SID to find participant
 * @returns {Promise<Object|null>} Participant object with status or null if not found
 */
const getParticipantStatus = async (callSid) => {
  const client = getClient();
  
  // Search through active conferences
  const conferences = await client.conferences.list({
    status: 'in-progress',
    limit: 20
  });
