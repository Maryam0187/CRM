import { NotificationService } from './sequelize-db.js';
...
export const NotificationManager = {
  ...
  async notifySaleStatusUpdated(data) {
    try {
      const { agentId, agentName, customerId, saleId, oldStatus, newStatus, customerName } = data;
      ...
      console.log(`📨 Sale status update notification sent to ${supervisors.length} supervisors`);
  ...
  async notifySaleCompleted(data) {
    try {
      ...
      console.log(`📨 Sale completion notification sent to ${supervisors.length} supervisors`);
      return { success: true, supervisorsCount: supervisors.length };
  ...
  async notifySaleCreated(data) {
    try {
      ...
      console.log(`📨 Sale creation notification sent to ${supervisors.length} supervisors`);
      return { success: true, supervisorsCount: supervisors.length };
