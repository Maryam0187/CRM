...
export function useActivityTracker({ inactivityTimeout = 5 * 60 * 1000, enabled = true } = {}) {
  ...
  const { isCalling, isWebCallConnected, callStatus } = useCall();
  const inactivityTimerRef = useRef(null);
  const isAwayRef = useRef(false);
  const lastActivityRef = useRef(Date.now());
  const isUpdatingRef = useRef(false); // Track if status update is in progress
  const activeCallStatuses = ['ringing', 'in-progress'];
  const isOnCall = isCalling || isWebCallConnected || activeCallStatuses.includes(callStatus);

  // Update status via API
  const updateStatus = useCallback(async (status) => {
    // Never mark agents away while they are actively handling a call.
    if (status === 'away' && isOnCall) {
      return;
    }
  ...
  }, [user, accessToken, socket, isConnected, isOnCall]);
  ...
  }, [inactivityTimeout, updateStatus, isOnCall]);
  ...
  }, [enabled, user, updateStatus, resetInactivityTimer, isOnCall]);
