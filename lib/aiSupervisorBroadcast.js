...
function broadcastMonitorAudio({ callSid, agentId, track, payload }) {
  ...
  const userRoom = `user_${parseInt(agentId, 10)}`;
  const callRoom = `call_${callSid}`;
  // Defer emit so OpenAI/Twilio WS handlers are not blocked by high-volume monitor packets.
  setImmediate(() => {
    io.to(callRoom).emit('ai_monitor_audio', event);
    io.to(userRoom).emit('ai_monitor_audio', event);
  });
}
