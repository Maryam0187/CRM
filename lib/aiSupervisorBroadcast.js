...
function broadcastMonitorAudio({ callSid, agentId, track, payload }) {
  ...
  // Initiator is always in user_* (joined on login). Call room alone can miss audio after reconnect.
  io.to(`user_${parseInt(agentId, 10)}`).emit('ai_monitor_audio', event);
}
