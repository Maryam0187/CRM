...
function initializeAiMediaBridge(server) {
  ...
  wss.on('connection', (twilioWs, request) => {
    ...
    function forceStartAiStreamManually() {
      ...
      return {
        ok: true,
        message: 'AI stream started manually',
    ...
    openAiWs.on('message', (data) => {
      try {
        ...
        if (event.type === 'session.updated') {
          ...
          if (streamSid && AI_AUTO_START_ON_ANSWER && isAiCallAnswered(runtimeCallSid)) {
...
function forceStartAiStreamForCall(callSid) {
  ...
  if (!hooks?.forceStart) {
    return {
      ok: false,
      message:
        'No active AI media stream for this call. The customer must be on the line (stream connected) before starting AI.'
    };
  }
