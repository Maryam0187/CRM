...
/** AI monitor lane throttle (customer audio is forwarded on every Twilio frame). */
...
function initializeAiMediaBridge(server) {
  ...
  wss.on('connection', (twilioWs, request) => {
    ...
    function maybeMonitorAudio(track, payload) {
      ...
      // Forward every customer frame — dropping 20ms packets causes choppy/distorted monitor audio.
      if (track !== 'customer') {
        const now = Date.now();
        if (now - lastMonitorAt[track] < MONITOR_AI_AUDIO_MS) return;
