...
/** Per-call hooks for the active Twilio↔OpenAI websocket session. */
const activeSessionHooksByCallSid = new Map();
/** Agent clicked Start AI Stream before Twilio WS connected — run when stream is ready. */
const pendingManualStartByCallSid = new Set();
      ...
      try {
        ...
        switch (message.event) {
          ...
          case 'media':
            ...
            // In standby: pipe customer audio to OpenAI (listen-only, no auto speech until Start AI Stream)
            sendJson(openAiWs, {
...
async function waitForAiStreamAndStart(callSid, maxWaitMs = 14000) {
  ...
  let lastMessage =
    'AI media stream did not connect in time. Set AI_MEDIA_STREAM_WS_URL to your WebSocket host (e.g. Railway wss URL).';
  ...
  while (Date.now() < deadline) {
    if (hasActiveAiMediaStream(sid)) {
      ...
      if (
        lastMessage.includes('OpenAI session is still starting') ||
        lastMessage.includes('not connected yet')
      ) {
        await sleep(400);
        continue;
      }
...
module.exports = {
  ...
  setAiControlAction(callSid, action, actorUserId = null) {
    ...
    if (normalized === 'start_stream' || normalized === 'start_ai') {
      ...
      console.log('[AI BRIDGE] Manual AI stream start', { callSid, ok: startResult.ok, message: startResult.message });
      return {
        ok: startResult.ok,
        state,
