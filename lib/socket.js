...
class SocketManager {
  ...
  async _getParticipantMonitoringModules() {
    ...
    try {
      ...
      // We don't need sequelizeDb for participant monitoring - we track calls in memory
  ...
  async updateAllActiveParticipants() {
    ...
    const { getConferenceParticipants } = modules;

    // Update participants for each active call
    // We already track active calls, so no need to query database
    for (const [callSid, callInfo] of this.activeCalls.entries()) {
      const { conferenceName, agentId } = callInfo;
      
      if (!conferenceName) {
        // Remove call if no conference name
        this.activeCalls.delete(callSid);
        continue;
      }
      ...
      try {
        ...
        console.error(`❌ Error fetching participants for ${callSid}:`, error.message);
        // Remove call from tracking if conference doesn't exist or call ended
    ...
    // Stop monitoring if no active calls remain
    if (this.activeCalls.size === 0) {
      this.stopParticipantMonitoring();
    }
  }
