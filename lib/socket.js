...
class SocketManager {
  ...
  async _getParticipantMonitoringModules() {
    ...
    try {
      // twilio.js is CommonJS - use require() with relative path (no .js extension)
      // Node.js will resolve it correctly
      const twilioModule = require(path.join(__dirname, 'twilio'));
      ...
      // sequelize-db.js is ES module - use dynamic import() with file:// URL for absolute path
      const sequelizeDbPath = path.resolve(__dirname, 'sequelize-db.js');
      const sequelizeDbUrl = `file://${sequelizeDbPath}`;
      const sequelizeDbModule = await import(sequelizeDbUrl);
      
      if (!sequelizeDbModule || !sequelizeDbModule.default) {
  ...
  async updateAllActiveParticipants() {
    ...
    let modules;
    try {
      modules = await this._getParticipantMonitoringModules();
    } catch (error) {
