...
class SocketManager {
  constructor() {
    ...
    // In-memory cache of latest call status, so agents who reconnect can receive any missed updates.
    // callSid -> last call_status_update payload
    this.latestCallStatusBySid = new Map();
    // agentId -> Map(callSid -> last call_status_update payload)
    this.activeCallStatusByAgent = new Map();
  }
  ...
  setupEventHandlers() {
    this.io.on('connection', (socket) => {
      ...
      // Replay latest active call statuses to this agent (helps recover from missed socket events).
      this.replayActiveCallStatuses(socket);
  ...
  // ===== Call status replay/cache =====
  cacheCallStatus(callSid, callStatusData) {
    if (!callSid || !callStatusData) return;
    this.latestCallStatusBySid.set(callSid, callStatusData);

    const agentId = callStatusData.agentId;
    if (!agentId) return;

    if (!this.activeCallStatusByAgent.has(agentId)) {
      this.activeCallStatusByAgent.set(agentId, new Map());
    }

    const statusForLifecycle = callStatusData.uiStatus || callStatusData.status;
    const endedStatuses = new Set(['completed', 'failed', 'canceled', 'busy', 'no-answer', 'voicemail']);

    if (endedStatuses.has(statusForLifecycle)) {
      // Remove from active calls for replay
      const m = this.activeCallStatusByAgent.get(agentId);
      if (m) m.delete(callSid);
    } else {
      this.activeCallStatusByAgent.get(agentId).set(callSid, callStatusData);
    }
  }

  replayActiveCallStatuses(socket) {
    try {
      if (!this.io || !socket) return;
      const agentId = socket.userId;
      const activeMap = this.activeCallStatusByAgent.get(agentId);
      if (!activeMap || activeMap.size === 0) return;

      for (const [callSid, cached] of activeMap.entries()) {
        socket.emit('call_status_update', cached);
      }
    } catch {
      // ignore
    }
  ...
  async handleDisconnect(socket) {
    ...
    // Do NOT invalidate the login session on socket disconnect - that would break page refresh.
    // Session invalidation only happens on explicit logout (auth/logout API) or admin force logout.
    
    // Check if user has any other connected sockets (presence) - use socket count, not DB sessions
    let otherSocketsForUser = 0;
    for (const [, s] of this.io.sockets.sockets) {
      if (s.userId === userId && s.id !== socket.id) otherSocketsForUser++;
    }
    
    // Only broadcast offline if no other socket connections exist for this user
    if (otherSocketsForUser === 0) {
      try {
        const { User } = require('../models');
        const user = await User.findByPk(userId);
        ...
        console.log(`📊 User ${userId} disconnected - no other sockets, broadcasting offline`);
      } catch (error) {
        console.error('Error updating status on disconnect:', error);
        this.broadcastUserStatusChange(userId, 'offline');
      }
    } else {
      console.log(`📊 User ${userId} disconnected - ${otherSocketsForUser} other socket(s) exist, not broadcasting offline`);
    }
  ...
  /**
   * Emit sale_updated so clients can refresh sales table without loader.
   * Sends to the sale's agent and to that agent's supervisors.
   * @param {number} saleId - Sale ID
   * @param {number} agentId - Agent (owner) of the sale
   * @param {number[]} supervisorIds - User IDs of supervisors of that agent
   */
  emitSaleUpdated(saleId, agentId, supervisorIds = []) {
    if (!this.io) {
      console.warn('⚠️ Socket.IO server not initialized, cannot emit sale_updated');
      return;
    }
    const payload = { saleId, agentId };
    if (agentId) {
      this.io.to(`user_${agentId}`).emit('sale_updated', payload);
    }
    const sent = new Set(agentId ? [agentId] : []);
    for (const uid of supervisorIds) {
      if (!sent.has(uid)) {
        this.io.to(`user_${uid}`).emit('sale_updated', payload);
        sent.add(uid);
      }
    }
    console.log(`📋 sale_updated emitted: saleId=${saleId}, agentId=${agentId}, recipients=${sent.size}`);
  }
  ...
  sendCallStatusUpdate(callSid, callData) {
    ...
    this.cacheCallStatus(callSid, callStatusData);

    // Send to all connected users (broadcast)
    this.io.emit('call_status_update', callStatusData);
    console.log(`📞 [socket] call_status_update broadcast for ${callSid}:`, {
      status: callData.status,
      uiStatus: callData.uiStatus,
      callbackType: callData.callbackType,
      webhookSource: callData.webhookSource,
  ...
  sendCallStatusToAgent(agentId, callSid, callData) {
    ...
    // Call room: agents who joined call_* still receive updates even if user_* mapping is stale
    this.io.to(`call_${callSid}`).emit('call_status_update', callStatusData);

    // Send to specific agent socket
    const socketId = this.connectedUsers.get(agentId);
    if (socketId) {
      this.io.to(socketId).emit('call_status_update', callStatusData);
      console.log(`📞 [socket] call_status_update sent to agent ${agentId} for ${callSid}:`, {
        status: callData.status,
        uiStatus: callData.uiStatus,
        callbackType: callData.callbackType,
        webhookSource: callData.webhookSource,
    ...
    console.log(`❌ Agent ${agentId} not connected, call status cached for replay`);
    return false;
  }
  ...
  // Send conference event updates (start, end, join, leave, mute, hold, speaker)
  sendConferenceEvent(conferenceName, eventData) {
    if (!this.io) {
      console.warn('⚠️ Socket.IO server not initialized, cannot send conference event');
      return false;
    }

    const conferenceEventData = {
    ...
    const conferenceRoom = `conference_${conferenceName}`;
    this.io.to(conferenceRoom).emit('conference_event', conferenceEventData);
    
    // Also send to call room if we have callSid
    if (eventData.callSid) {
      const callRoom = `call_${eventData.callSid}`;
      this.io.to(callRoom).emit('conference_event', conferenceEventData);
    }
    
    // Broadcast to all supervisors
    this.io.to('supervisors').emit('conference_event', conferenceEventData);
    
    // Also broadcast to all (for components that need it)
    this.io.emit('conference_event', conferenceEventData);
    
    console.log(`📞 Conference event "${eventData.event}" sent for conference: ${conferenceName}`);
    return true;
  }

  // Send conference status update (includes conference status, participant count, etc.)
  sendConferenceStatus(conferenceName, statusData) {
    if (!this.io) {
      console.warn('⚠️ Socket.IO server not initialized, cannot send conference status');
      return false;
    }

    const conferenceStatusData = {
    ...
    // Send to conference room
    const conferenceRoom = `conference_${conferenceName}`;
    this.io.to(conferenceRoom).emit('conference_status', conferenceStatusData);
    
    // Also send to call room if we have callSid
    if (statusData.callSid) {
      const callRoom = `call_${statusData.callSid}`;
      this.io.to(callRoom).emit('conference_status', conferenceStatusData);
    }
    
    // Broadcast to all supervisors
    this.io.to('supervisors').emit('conference_status', conferenceStatusData);
    
    // Also broadcast to all (for components that need it)
    this.io.emit('conference_status', conferenceStatusData);
    
    console.log(`📞 Conference status sent for conference: ${conferenceName}`, statusData);
    return true;
  }
  ...
  joinCallRoom(socket, callSid) {
    ...
    const cached = this.latestCallStatusBySid.get(callSid);
    if (cached) {
      socket.emit('call_status_update', cached);
      console.log(`📞 Replayed cached call status to user ${socket.userId} for ${callSid}:`, {
        status: cached.status,
        uiStatus: cached.uiStatus
      });
    }
