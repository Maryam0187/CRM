...
class SocketManager {
  ...
  async _getParticipantMonitoringModules() {
    ...
    try {
      ...
      // sequelize-db.js is ES module - use dynamic import() with relative path (same pattern as app/api/sales/route.js)
      let sequelizeDb;
      try {
        const sequelizeDbModule = await import('./sequelize-db.js');
        sequelizeDb = sequelizeDbModule.default;
      } catch (e) {
        console.warn('⚠️ Could not import sequelize-db, participant monitoring will work without DB verification');
        sequelizeDb = null;
      }
