...
export const SaleService = {
      ...
      include: [
        {
          model: Customer,
          as: 'customer',
          attributes: ['firstName', 'phone', 'landline','address','state','city','country','mailingAddress','customerFeedback']
        },
        {
          model: User,
          as: 'agent',
  ...
  async findAllPaginated(page = 1, limit = 10, numberSearch = null, searchLastFour = false) {
  ...
  async findByStatusPaginated(statuses, page = 1, limit = 10, numberSearch = null, searchLastFour = false) {
  ...
  async findByDatePaginated(dateFilter, page = 1, limit = 10, dateField = 'created_at', numberSearch = null, searchLastFour = false) {
    const { startDate, endDate } = this.parseDateFilter(dateFilter, dateField);
  ...
  async findByStatusAndDatePaginated(statuses, dateFilter, page = 1, limit = 10, dateField = 'created_at', numberSearch = null, searchLastFour = false) {
    const { startDate, endDate } = this.parseDateFilter(dateFilter, dateField);
  ...
  async findByAgentPaginated(agentId, page = 1, limit = 10, numberSearch = null, searchLastFour = false) {
    return await this.findWithPagination({
      where: { agentId },
      page,
      limit,
  ...
  async findByAgentStatusPaginated(agentId, statuses, page = 1, limit = 10, numberSearch = null, searchLastFour = false) {
    // Support both single status (string) and multiple statuses (array)
    const statusArray = Array.isArray(statuses) ? statuses : [statuses];
    const statusCondition = statusArray.length === 1 ? statusArray[0] : { [Op.in]: statusArray };
    
    return await this.findWithPagination({
      where: { agentId, status: statusCondition },
      page,
      limit,
      numberSearch,
      searchLastFour,
  ...
  async findByAgentDatePaginated(agentId, dateFilter, page = 1, limit = 10, dateField = 'created_at', numberSearch = null, searchLastFour = false) {
    const { startDate, endDate } = this.parseDateFilter(dateFilter, dateField);
    return await this.findWithPagination({
      where: { 
        agentId,
  ...
  async findByAgentStatusAndDatePaginated(agentId, statuses, dateFilter, page = 1, limit = 10, dateField = 'created_at', numberSearch = null, searchLastFour = false) {
    const { startDate, endDate } = this.parseDateFilter(dateFilter, dateField);
