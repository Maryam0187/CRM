...
import { getUtcBoundsForLocalDateRange, getUserLocalTodayDateString } from './dateFilterTimezone.js';
...
export const SaleService = {
  ...
  async findByDate(dateFilter, dateField = 'created_at', tzOffsetMinutes = 0) {
    const { startDate, endDate } = this.parseDateFilter(dateFilter, dateField, tzOffsetMinutes);
  ...
  async findByStatusAndDate(status, dateFilter, dateField = 'created_at', tzOffsetMinutes = 0) {
    const { startDate, endDate } = this.parseDateFilter(dateFilter, dateField, tzOffsetMinutes);
  ...
  async findAppointmentsForDashboard(agentId, dateFilter, dateField = 'appointmentDateTime', tzOffsetMinutes = 0) {
    const { startDate, endDate } = this.parseDateFilter(dateFilter, dateField, tzOffsetMinutes);
  ...
  parseDateFilter(dateFilter, dateField = 'created_at', tzOffsetMinutes = 0) {
    let startDate = null;
    let endDate = null;
    ...
    if (dateFilter === 'today') {
      const today = getUserLocalTodayDateString(tzOffsetMinutes);
      const bounds = getUtcBoundsForLocalDateRange(today, today, tzOffsetMinutes);
      startDate = bounds.startDate;
      endDate = bounds.endDate;
    } else if (dateFilter === 'yesterday') {
      const todayLocal = new Date(`${getUserLocalTodayDateString(tzOffsetMinutes)}T00:00:00.000Z`);
      todayLocal.setUTCDate(todayLocal.getUTCDate() - 1);
      const yesterday = `${todayLocal.getUTCFullYear()}-${String(todayLocal.getUTCMonth() + 1).padStart(2, '0')}-${String(todayLocal.getUTCDate()).padStart(2, '0')}`;
      const bounds = getUtcBoundsForLocalDateRange(yesterday, yesterday, tzOffsetMinutes);
      startDate = bounds.startDate;
      endDate = bounds.endDate;
    } else if (dateFilter === 'week') {
      const todayLocal = new Date(`${getUserLocalTodayDateString(tzOffsetMinutes)}T00:00:00.000Z`);
      const weekStartLocal = new Date(todayLocal);
      weekStartLocal.setUTCDate(weekStartLocal.getUTCDate() - 6);
      const weekStart = `${weekStartLocal.getUTCFullYear()}-${String(weekStartLocal.getUTCMonth() + 1).padStart(2, '0')}-${String(weekStartLocal.getUTCDate()).padStart(2, '0')}`;
      const today = `${todayLocal.getUTCFullYear()}-${String(todayLocal.getUTCMonth() + 1).padStart(2, '0')}-${String(todayLocal.getUTCDate()).padStart(2, '0')}`;
      const bounds = getUtcBoundsForLocalDateRange(weekStart, today, tzOffsetMinutes);
      startDate = bounds.startDate;
      endDate = bounds.endDate;
      ...
      const monthEndDate = new Date(Date.UTC(Number(year), monthIndex + 1, 0));
      ...
      const bounds = getUtcBoundsForLocalDateRange(monthStart, monthEnd, tzOffsetMinutes);
      startDate = bounds.startDate;
      endDate = bounds.endDate;
  ...
  async findByDatePaginated(dateFilter, page = 1, limit = 10, dateField = 'created_at', numberSearch = null, searchLastFour = false, idSearch = null, tzOffsetMinutes = 0) {
    const { startDate, endDate } = this.parseDateFilter(dateFilter, dateField, tzOffsetMinutes);
  ...
  async findByStatusAndDatePaginated(statuses, dateFilter, page = 1, limit = 10, dateField = 'created_at', numberSearch = null, searchLastFour = false, idSearch = null, tzOffsetMinutes = 0) {
    const { startDate, endDate } = this.parseDateFilter(dateFilter, dateField, tzOffsetMinutes);
  ...
  async findByAgentDatePaginated(agentId, dateFilter, page = 1, limit = 10, dateField = 'created_at', numberSearch = null, searchLastFour = false, idSearch = null, tzOffsetMinutes = 0) {
    const { startDate, endDate } = this.parseDateFilter(dateFilter, dateField, tzOffsetMinutes);
    return await this.findWithPagination({
      where: { 
        agentId,
  ...
  async findByAgentDate(agentId, dateFilter, dateField = 'created_at', tzOffsetMinutes = 0) {
    const { startDate, endDate } = this.parseDateFilter(dateFilter, dateField, tzOffsetMinutes);
  ...
  async findByAgentStatusAndDatePaginated(agentId, statuses, dateFilter, page = 1, limit = 10, dateField = 'created_at', numberSearch = null, searchLastFour = false, idSearch = null, tzOffsetMinutes = 0) {
    const { startDate, endDate } = this.parseDateFilter(dateFilter, dateField, tzOffsetMinutes);
...
export const RoleAssignmentService = {
  ...
  async findByAgentStatusAndDate(agentId, status, dateFilter, dateField = 'created_at', tzOffsetMinutes = 0) {
    const { startDate, endDate } = this.parseDateFilter(dateFilter, dateField, tzOffsetMinutes);
