...
import { formatNumericOnly } from '../lib/validation.js';
import apiClient from '../lib/apiClient.js';

export default function AddChequeMailForm({ mode, saleId, customerId, onSuccess, initialData, onDataChange }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!formData.chequeNumber || !formData.nameOnCheque || !formData.bankName) {
      ...
      const response = await apiClient.post('/api/cheques-mail', chequeData);
      ...
      onSuccess({
        type: 'cheque_mail',
        data: result.data,
        status: 'cheque_mail_added',
        message: 'Cheque to mail details added successfully'
      });
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      >
        {isSubmitting ? 'Adding...' : 'Add Cheque to Mail'}
      </button>
    </form>
  );
}
